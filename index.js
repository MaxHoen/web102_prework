...
const unfundedGames = GAMES_JSON.filter(game => game.pledged < game.goal);
const gamesUnfundedlen = unfundedGames.length;


// create a string that explains the number of unfunded games using the ternary operator
const summary = `$${totalRaised.toLocaleString()} has been raised for 11 games. Currently, ${gamesUnfundedlen} game${gamesUnfundedlen !== 1 ? "s" : ""} remain${gamesUnfundedlen !== 1 ? "" : "s"} unfunded. Help us fund these amazing games!`;
...
const [firstTopGame, secondTopGame] = sortedGames; 

// create a new element to hold the name of the top pledge game, then append it to the correct element
const firstGameElement = document.createElement("p");
firstGameElement.innerText = firstTopGame.name;
